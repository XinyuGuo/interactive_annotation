...
def sim_clicks_init_batch(img_ids, data, mask, mask_meta, model, device):
    '''
...
def sim_initial_click_dataset_batch(dataloader, model_init, sigma, device):
    '''
...
def sim_clicks(batch_data, model, clicks_info, sigma, device, click_num):
    ...
    train_data_1_c_mask = batch_data['pred_mask'].to(device)
...
def sim_clicks_batch(sim_batch, clicks_info, model, num_points, sigma, device):
    key_list = sim_batch['kidney_id']
    for k in range(num_points):
        # print(k)
        clicks_list = sim_clicks(sim_batch, model, clicks_info, sigma, device, k) 
        fill_the_dict(clicks_info, clicks_list, key_list)

def sim_clicks_dataset_batch(dataloader,clicks_info_init,model,num_points,sigma,device):
    '''
    simulate  clicks during the trianing
    '''
    model.eval()
    for sim_batch in tqdm(dataloader):
        sim_clicks_batch(sim_batch, clicks_info_init, model, num_points, sigma, device)
    model.train()
    return clicks_info_init
